...
// --- START: NEW SKELETON COMPONENTS ---

const ProductCardSkeleton = () => (
    <div className="bg-[#1a1c23] border border-[#2a2d36] rounded-lg overflow-hidden flex flex-col">
        <Skeleton className="aspect-square w-full bg-[#2a2d36]" />
        <div className="p-4 flex flex-col flex-grow">
            <div className="h-12 mb-2">
                <Skeleton className="h-4 w-5/6 bg-[#2a2d36] mb-2" />
            ...
            <div className="flex justify-between items-center mt-auto">
                ...
                <Skeleton className="h-6 w-10 bg-[#2a2d36] rounded-full" />
            </div>
        </div>
    </div>
);
...
export default function ProductsPage() {
    ...
    // --- START OF MODIFICATION: Add transitioning and initial load states ---
    ...
    // --- START OF MODIFICATION: Refined loading state management ---
    useEffect(() => {
        if (isFilterableGridMode) {
            // The global loading screen is now only for the very first page load.
            if (isInitialLoad && isGridLoading) {
            ...
            // The local `isTransitioning` state handles all subsequent data fetches.
            setIsTransitioning(isGridLoading);

            // Once the first successful load completes, turn off the initial load flag.
            if (isInitialLoad && !isGridLoading && gridResults.length > 0) {
                setIsInitialLoad(false);
            }
        }
    }, [isGridLoading, isFilterableGridMode, gridResults.length, isInitialLoad, setIsLoadingProducts, setLoadingMessage]);
    // --- END OF MODIFICATION ---
        ...
        return (
            <div className="min-h-screen bg-[#0f1115] text-white">
                ...
                <div className="container mx-auto px-4 py-8">
                                ...
                                <Sheet open={isFilterSheetOpen} onOpenChange={setIsFilterSheetOpen}>
                                    ...
                                    <SheetContent side="left" className="bg-[#1a1c23] border-r border-[#2a2d36] text-white w-[85vw] max-w-md p-0 flex flex-col">
                                        ...
                                        <ScrollArea className="flex-1 px-4 py-2 dark-theme-scrollbar">
                                            <FilterContent 
                                                showCategoryFilter={false} showSubcategoryFilter={false} showLabelFilter={false} filterGroups={filterGroups}
                                                activeCategories={[]} activeSubcategories={[]} activeLabels={[]} activeConditions={activeConditions}
                                                toggleFilterOption={toggleFilterOption} handleClearFilters={handleClearFilters}
                                                isDiscountFilterEnabled={isDiscountFilterEnabled} handleDiscountFilterToggle={handleDiscountFilterToggle}
                                                discountPriceRange={discountPriceRange} handlePriceRangeChange={handlePriceRangeChange}
                                                onApplyPriceFilter={onApplyPriceFilter} discountPriceBounds={discountPriceBounds}
                                                dynamicSpecFilters={dynamicSpecFilters} activeTechSpecs={activeTechSpecs}
                                                toggleTechSpecFilter={toggleTechSpecFilter} handleTechSpecRangeChange={handleTechSpecRangeChange} 
                                            />
                                        </ScrollArea>
                    ...
                    <ActiveFilters 
                        activeCategories={[]} activeSubcategories={[]} activeLabels={[]} activeConditions={activeConditions} 
                        isDiscountFilterEnabled={isDiscountFilterEnabled} discountPriceRange={discountPriceRange} 
                        toggleFilterOption={toggleFilterOption} handleClearFilters={handleClearFilters} handleDiscountFilterToggle={handleDiscountFilterToggle} 
                        activeTechSpecs={activeTechSpecs} toggleTechSpecFilter={toggleTechSpecFilter} handleTechSpecRangeChange={handleTechSpecRangeChange} 
                    />
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-1">
                             {/* --- START OF MODIFICATION: Render logic for skeletons --- */}
                             {isTransitioning ? (
                                <ProductGridSkeleton />
                            ) : gridResults.length > 0 ? (
                                <>
                                    <div className="grid grid-cols-2 xs:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                                        {gridResults.map((product) => (<ProductCard key={product.slug} product={product} />))}
                                    </div>
