...
interface FilterContentProps {
  ...
  toggleFilterOption: (
    filterType: "category" | "subcategory" | "label" | "condition",
    option: string
  ) => void;
    ...
    <div className="space-y-6">
      <Accordion type="single" collapsible defaultValue="categories" className="border-none">
        ...
        <AccordionItem value="conditions" className="border-b border-[#2a2d36]">
          <AccordionTrigger className="text-sm font-medium py-2 hover:no-underline">
            <span className={activeConditions.length > 0 ? "text-[#5865f2]" : "text-white"}>
              Conditions
            </span>
          </AccordionTrigger>
          <AccordionContent>
            <div className="space-y-2 max-h-64 overflow-y-auto pr-2 dark-theme-scrollbar">
              {filterGroups.conditions.length > 0 ? (
                  ...
                  <div key={cond.name} className="flex items-center justify-between">
                    <div className="flex items-center">
                      <Checkbox
                        ...
                        checked={activeConditions.includes(cond.name)}
                        onCheckedChange={() => toggleFilterOption("condition", cond.name)}
                      />
                      <label
                        htmlFor={`condition-${cond.name}`}
                        className={`ml-2 text-sm cursor-pointer ${
                          activeConditions.includes(cond.name) ? "text-[#5865f2]" : "text-gray-300"
                        }`}
                      >
                        {cond.name}
                      </label>
                ...
                <p className="text-xs text-gray-400">No conditions available</p>
              )}
            </div>
          </AccordionContent>
        </AccordionItem>
              ...
              <div className={isDiscountFilterEnabled ? "" : "opacity-50 pointer-events-none"}>
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-gray-300">Min: {formatCurrencyWithSeparator(discountPriceRange[0])}</span>
                    <span className="text-sm text-gray-300">Max: {formatCurrencyWithSeparator(discountPriceRange[1])}</span>
                  </div>
                  <Slider
  ...
  return (
    <div className="flex flex-wrap gap-2 my-4">
      ...
      {activeConditions.map((cond) => (
        <Badge key={`cond-${cond}`} className="bg-[#5865f2] text-white py-1 px-2 flex items-center gap-1">
          Condition: {cond}
          <button onClick={() => toggleFilterOption("condition", cond)} className="ml-1">
            <X className="h-3 w-3" />
          </button>
        </Badge>
      ))}
      {isDiscountFilterEnabled && (
        <Badge className="bg-[#5865f2] text-white py-1 px-2 flex items-center gap-1">
          Price: {formatCurrencyWithSeparator(discountPriceRange[0])} - {formatCurrencyWithSeparator(discountPriceRange[1])}
