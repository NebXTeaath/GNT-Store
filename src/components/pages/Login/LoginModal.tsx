...
const CheckEmailView = ({ email, onClose }: { email: string; onClose: () => void }) => {
    ...
    return (
        <div className="p-6 text-center space-y-4">
            ...
            <Button onClick={handleResendEmail} variant="outline" className="w-full mt-2 border-[#5865f2] text-[#5865f2] hover:bg-[#5865f2]/10 disabled:opacity-50 hover:text-white" disabled={!canResend || isResending}> {isResending ? ( <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Sending...</> ) : canResend ? ( "Resend Verification Email" ) : ( `Resend in ${countdown}s` )} </Button>
