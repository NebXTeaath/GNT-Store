            ...
            <Tabs value={activeTab} onValueChange={handleTabChangeInternal} className="w-full">
                ...
                <TabsContent value="register">
                     <form onSubmit={handleRegisterSubmit} className="space-y-4">
                         ...
                         {error && <p className="text-red-500 text-sm flex items-center"><AlertCircle className="h-4 w-4 mr-1"/>{error}</p>}
                         <Button type="submit" className="w-full bg-[#5865f2]" disabled={isLoading||isResending}> {isLoading?(<><Loader2 className="spin mr-2 h-4 w-4 animate-spin"/>Registering...</>):(<><UserPlus className="mr-2 h-4 w-4"/>Register</>)} </Button>
