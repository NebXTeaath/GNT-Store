...
import { Carousel, CarouselApi, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from "@/components/ui/carousel"; // Added CarouselApi type
...
// Assuming the hook is now correctly located here:
import { useProductDetails } from "@/pages/ProductDetails/useProductDetails"; // Use central hooks directory path
...
export function DynamicBreadcrumb() {
    ...
    const { productData, isLoading, error } = useProductDetails(id);

    useEffect(() => {
        if (productData) { document.title = `[GNT] ${productData.o_product_name}`; }
        else if (isLoading) { document.title = "[GNT] Loading Product..."; }
        else if (error) { document.title = "[GNT] Product Error"; }
        else { document.title = "[GNT] Product Not Found"; }
        return () => { document.title = "GNT"; };
    ...
    if (isLoading && !productData) {
        return ( <Breadcrumb className="mb-4"> <BreadcrumbList> <BreadcrumbItem><Skeleton className="h-5 w-12" /></BreadcrumbItem> <BreadcrumbSeparator /> <BreadcrumbItem><Skeleton className="h-5 w-20" /></BreadcrumbItem> <BreadcrumbSeparator /> <BreadcrumbItem><Skeleton className="h-5 w-16" /></BreadcrumbItem> </BreadcrumbList> </Breadcrumb> );
    }

    return ( <Breadcrumb className="mb-4"> <BreadcrumbList> {breadcrumbItems.map((item, index) => { const isLast = index === breadcrumbItems.length - 1; return ( <React.Fragment key={`<span class="math-inline">\{item\.label\}\-</span>{index}`}> {index > 0 && <BreadcrumbSeparator />} <BreadcrumbItem> {isLast || !item.href ? ( <BreadcrumbPage className="text-white">{item.label}</BreadcrumbPage> ) : ( <BreadcrumbLink href={item.href} className="hover:text-[#5865f2] transition-colors">{item.label}</BreadcrumbLink> )} </BreadcrumbItem> </React.Fragment> ); })} </BreadcrumbList> </Breadcrumb> );
}
...
export default function ProductDetailsPage() {
    ...
    const { productData, isLoading, error, isFetching } = useProductDetails(id);

    const [quantity, setQuantity] = useState<number | null>(null);
    const [isNameExpanded, setIsNameExpanded] = useState(false);
    const [api, setApi] = React.useState<CarouselApi>() // State for Carousel API
    const [currentSlide, setCurrentSlide] = useState(0); // Keep track of slide index
    ...
    // Effect for Carousel API to update slide index state
     useEffect(() => {
        if (!api) { return }
        setCurrentSlide(api.selectedScrollSnap()) // Set initial snap
        api.on("select", () => { setCurrentSlide(api.selectedScrollSnap()) }) // Update on select
    }, [api])
    ...
    const decreaseQuantity = () => { /* ... unchanged ... */
    ...
    const increaseQuantity = () => { /* ... unchanged ... */
    ...
    const handleAddToCart = () => { /* ... unchanged ... */
        ...
        if (productData && quantity !== null && quantity > 0) {
            if (cartItems.length >= 20 && !cartItems.find(item => item.id === productData.o_product_id)) {
                toast.error("Cart is full! Maximum 20 items allowed.", { id: "cart-full-toast" }); return;
            }
            addToCart({ id: productData.o_product_id, title: productData.o_product_name, price: parseFloat(productData.o_price), discount_price: parseFloat(productData.o_discount_price), image: productData.o_images?.[0]?.[0]?.url || "" }, quantity);
        } else {
    ...
    const handleToggleWishlist = () => { /* ... unchanged ... */
        if (!isAuthenticated) { setLoginModalOpen(true); return; }
        if (productData) {
            ...
            else { addToWishlist({ id: productData.o_product_id, title: productData.o_product_name, price: parseFloat(productData.o_price), discount_price: parseFloat(productData.o_discount_price), image: productData.o_images?.[0]?.[0]?.url || "" }); }
        }
    };

    // Mobile footer visibility logic (Unchanged)
    const checkMobileFooterVisibility = useCallback(() => { /* ... unchanged ... */
        if (window.innerWidth >= 768) { setShowMobileFooter(false); return; }
        if (!mainContentRef.current || !addToCartSectionRef.current) return;
        const windowHeight = window.innerHeight; const addToCartRect = addToCartSectionRef.current.getBoundingClientRect(); const shouldShowFooter = addToCartRect.bottom < 0 || addToCartRect.top > windowHeight; setShowMobileFooter(shouldShowFooter);
    }, []);
    useEffect(() => { /* ... unchanged ... */
        let ticking = false; const throttledCheck = () => { if (!ticking) { window.requestAnimationFrame(() => { checkMobileFooterVisibility(); ticking = false; }); ticking = true; } }; window.addEventListener("scroll", throttledCheck, { passive: true }); window.addEventListener("resize", throttledCheck); throttledCheck(); return () => { window.removeEventListener("scroll", throttledCheck); window.removeEventListener("resize", throttledCheck); };
    }, [checkMobileFooterVisibility]);
    useLayoutEffect(() => { /* ... unchanged ... */
        const timer = setTimeout(() => { checkMobileFooterVisibility(); }, 200); return () => clearTimeout(timer);
    ...
    if (isLoading && !productData) {
        // ... (Skeleton rendering remains the same) ...
         return ( <div className="min-h-screen bg-[#0f1115] text-white font-sans overflow-x-hidden relative"> <main ref={mainContentRef} className="container mx-auto px-4 py-4 max-w-7xl"> <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}> <DynamicBreadcrumb /> </motion.div> <div className="grid lg:grid-cols-2 gap-x-8 xl:gap-x-16 gap-y-8"> <motion.div initial="hidden" animate="visible" variants={fadeIn}> <ProductImageSkeleton /> </motion.div> <motion.div initial="hidden" animate="visible" variants={fadeIn} transition={{ delay: 0.1 }}> <ProductDetailsSkeleton /> </motion.div> </div> <motion.div initial="hidden" animate="visible" variants={fadeIn} transition={{ delay: 0.2 }} className="mt-16"> <SimilarProductsSkeleton /> </motion.div> </main> </div> );
    }

    // Error State
     if (error) { // Separate check for error
        return ( <div className="min-h-screen bg-[#0f1115] text-white font-sans overflow-x-hidden flex flex-col items-center justify-center p-4"> <motion.div initial="hidden" animate="visible" variants={fadeIn}> <div className="container mx-auto px-4 py-4 max-w-7xl self-start"> <DynamicBreadcrumb /> </div> <div className="text-center mt-8"> <h2 className="text-2xl font-bold mb-4">Error</h2> <p className="text-lg text-gray-300 mb-6">{error}</p> <Button onClick={() => navigate("/")} className="bg-[#5865f2] hover:bg-[#4752c4]">Go Home</Button> </div> </motion.div> </div> );
    }

    // Not Found State (Loading finished, but no data)
    if (!isLoading && !productData) {
        return ( <div className="min-h-screen bg-[#0f1115] text-white font-sans overflow-x-hidden flex flex-col items-center justify-center p-4"> <motion.div initial="hidden" animate="visible" variants={fadeIn}> <div className="container mx-auto px-4 py-4 max-w-7xl self-start"> <DynamicBreadcrumb /> </div> <div className="text-center mt-8"> <h2 className="text-2xl font-bold mb-4">Product Not Found</h2> <p className="text-lg text-gray-300 mb-6">The product you are looking for does not exist or could not be loaded.</p> <Button onClick={() => navigate("/")} className="bg-[#5865f2] hover:bg-[#4752c4]">Go Home</Button> </div> </motion.div> </div> );
    }


    // --- Render Product Details: Add explicit check ---
    // If execution reaches here, isLoading is false, error is null, and productData exists.
    // This explicit check removes any ambiguity for TypeScript.
    if (productData) {
                        ...
                        <motion.div variants={fadeIn} initial="hidden" animate="visible" className="space-y-4">
                            <div className="flex flex-col gap-8">
                                <div className="relative overflow-visible -mx-4 md:mx-0 md:px-12">
                                    {/* Pass setApi to the Carousel */}
                                    <Carousel setApi={setApi} index={currentSlide} opts={{ loop: images.length > 1 }} /* Added loop */ >
                                        <CarouselContent>
                                            {images.map((img, index) => (
                                                <CarouselItem key={index}>
                                                    <div className="aspect-square relative overflow-hidden rounded-xl bg-gray-800">
                                                        <img src={img || "/placeholder.svg"} alt={`${productData.o_product_name} image ${index + 1}`} className="w-full h-full object-contain" loading="lazy"/>
                                                    </div>
                                                </CarouselItem>
                                            ))}
                                        </CarouselContent>
                                ...
                                {images.length > 1 && (
                                    <div className="grid grid-cols-4 gap-2 md:gap-4">
                                        {images.slice(0, 4).map((img, index) => (
                                            <motion.div
                                                key={index} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: index * 0.1 }}
                                                // Use api?.scrollTo(index) for thumbnail click
                                                onClick={() => api?.scrollTo(index)}
                                                className={`aspect-square relative overflow-hidden rounded-lg bg-gray-800 cursor-pointer transition-all duration-300 ${ currentSlide === index ? "ring-2 ring-[#5865f2]" : "hover:ring-2 hover:ring-[#5865f2]/50" }`}
                                            >
                                                <img src={img || "/placeholder.svg"} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover" loading="lazy"/>
                                            </motion.div>
                                        ))}
                                    </div>
                                )}
                                        ...
                                        {productData.o_similar_products.map((product: SimilarProduct, index) => {
                                            ...
                                            return (
                                                <motion.div key={product.product_id} variants={fadeIn} custom={index} transition={{ delay: index * 0.05 }} onClick={() => navigate(`/product/${product.product_id}`)} className="bg-[#1a1c23] border border-[#2a2d36] rounded-lg overflow-hidden hover:border-[#5865f2] transition-all duration-300 cursor-pointer flex flex-col group" whileHover={{ y: -5, boxShadow: "0 10px 15px -3px rgba(88, 101, 242, 0.2), 0 4px 6px -4px rgba(88, 101, 242, 0.1)" }}>
                                                    <div className="aspect-square relative w-full"><img src={product.primary_image || "/placeholder.svg"} alt={product.product_name} className="w-full h-full object-cover" loading="lazy"/><div className="absolute top-2 right-2 flex flex-col gap-1">{product.is_bestseller && (<div className="bg-[#EFBF04] text-[#1f10f7] font-bold text-xs px-1.5 py-0.5 rounded border border-[#EFBF04] shadow-sm">Pop</div>)}{similarPrice > similarDiscountPrice && (<div className="bg-[#ff4d4d] text-white text-xs font-semibold px-1.5 py-0.5 rounded shadow-sm">{calculateDiscountPercentage(similarPrice, similarDiscountPrice)}%</div>)}</div><div className="absolute bottom-2 left-2 bg-[#1a1c23]/80 backdrop-blur-sm text-white text-xs px-1.5 py-0.5 rounded border border-[#2a2d36]">{capitalize(product.condition)}</div></div>
