...
// Added local implementations for getStatusColor and formatRepairStatus
const getStatusColor = (status: string): string => {
    switch(status) {
      case "pending": return "bg-yellow-500";
      case "received": return "bg-blue-500";
      case "diagnosing": return "bg-indigo-500";
...
export default function History({ onViewDetails }: HistoryProps) {
  ...
  useEffect(() => {
    document.title = "[GNT] Repair History";
  }, []);
  ...
  const fetchRepairHistory = async () => {
    if (!userProfileData?.userId) return;
      ...
      {isHistoryLoading ? (
        ...
        <div className="space-y-4">
          {historyRequests.map((request) => (
            <Card className="!bg-[#2a2d36] border-[#3f4354] overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row">
                <div className="md:w-64 bg-[#1a1c23] p-4 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="font-medium mb-1 text-sm">{request.productModel}</h3>
