...
export default function SearchPage() {
    ...
    const [discountPriceRange, setDiscountPriceRange] = useState<[number, number]>(
        () => {
            const min = Number(searchParams.get("minDiscountPrice")) || 0;
            const max = Number(searchParams.get("maxDiscountPrice")) || 0;
    ...
    const {
        ...
        filterGroups, discountPriceBounds, // Get bounds from the hook
        didYouMean,
        ...
        discountPriceRange: debouncedPriceRange, // Pass debounced value for filtering
    });

    // --- Handlers defined in the Page component ---
    const updateFilters = useCallback(/* ... unchanged ... */ (updates: Record<string, string | null>) => {
    ...
    const toggleFilterOption = useCallback(/* ... unchanged ... */ (filterKey: "category" | "subcategory" | "label" | "condition", option: string) => {
        const currentParam = searchParams.get(filterKey); let currentArray = currentParam ? currentParam.split(",") : []; const optionIndex = currentArray.indexOf(option);
        if (optionIndex > -1) { currentArray.splice(optionIndex, 1); } else { currentArray.push(option); }
        updateFilters({ [filterKey]: currentArray.length > 0 ? currentArray.join(",") : null });
    }, [searchParams, updateFilters]);

    // --- FIX: Modified handleDiscountFilterToggle ---
    const handleDiscountFilterToggle = useCallback((checked: boolean) => {
        setIsDiscountFilterEnabled(checked); // Update local enabled state

        if (checked) {
            // 1. Immediately set local range state to full bounds
            // Ensures slider visually starts at full range
            setDiscountPriceRange(discountPriceBounds);

            // 2. Update URL using the full bounds
            updateFilters({
                filterByDiscount: "true",
                minDiscountPrice: discountPriceBounds[0].toString(), // Use bounds for URL
                maxDiscountPrice: discountPriceBounds[1].toString(), // Use bounds for URL
            });
        } else {
            // When disabling, remove discount params from URL
            updateFilters({
                filterByDiscount: null,
                minDiscountPrice: null,
                maxDiscountPrice: null,
            });
            // Also reset local range state to bounds for consistency next time
            setDiscountPriceRange(discountPriceBounds);
        }
        // Add discountPriceBounds to dependencies
    }, [updateFilters, discountPriceBounds]);

    // Update local price range state (slider/input changes) - Unchanged
    ...
    // Clear all filters - Unchanged (already resets range to bounds)
    const handleClearFilters = useCallback(() => {
        updateFilters({ category: null, subcategory: null, label: null, condition: null, filterByDiscount: null, minDiscountPrice: null, maxDiscountPrice: null, page: "1", sortBy: "relevance" });
        setIsDiscountFilterEnabled(false);
        setDiscountPriceRange(discountPriceBounds);
    }, [updateFilters, discountPriceBounds]);

    // Handle suggestion click - Unchanged
    const handleSuggestionClick = useCallback((suggestion: string) => { updateFilters({ q: suggestion, page: "1" }); }, [updateFilters]);

    // Handle sort selection change - Unchanged
    ...
    // --- Active Filters Check --- (Unchanged)
...
return (
    <div className="min-h-screen bg-[#0f1115] text-white">
        <div className="container mx-auto px-4 py-8">
            {/* Top section with suggestions, count, filters, sort */}
            <div className="mb-6">
                {/* ... (Did you mean, Count, Filter/Sort buttons) ... */}
                {didYouMean && (
                    <p className="text-sm text-gray-400 mb-2">
                        Did you mean: <button onClick={() => handleSuggestionClick(didYouMean)} className="ml-1 text-[#5865f2] hover:underline">{didYouMean}</button>
                    </p>
                )}
                    ...
                    <div className="flex items-center gap-4">
                        {isMobile ? (
                            ...
                            <Sheet>
                                ...
                                {/* Desktop SheetContent if needed */}
                            </Sheet>
                        )}
            ...
            <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-64 lg:w-72 flex-shrink-0">
                        <div className="bg-[#1a1c23] border border-[#2a2d36] rounded-lg p-4 sticky top-20">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <Button variant="ghost" size="sm" onClick={handleClearFilters} className="h-8 px-2 text-xs hover:bg-[#2a2d36]">
                ...
                <div className="flex-1">
                    {/* ... (Loading / Results / No Results / Pagination rendering) ... */}
                    {isLoading ? (
