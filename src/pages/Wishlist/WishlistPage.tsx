...
const calculateDiscountPercentage = (originalPrice: number, discountPrice: number): number => { return Math.round(((originalPrice - discountPrice) / originalPrice) * 100); };
...
export default function WishlistPage() {
  ...
  const navigateToProduct = (slug: string) => { if (slug) { navigate(`/product/${slug}`); } else { toast.error("Product details not available", { id: "product-not-found", description: "The product information could not be found." }); } };

  // ----- Effects -----
  useEffect(() => { window.scrollTo({ top: 0, behavior: "smooth" }); }, []);
  useEffect(() => { if (currentPage > totalPages && totalPages > 0) { setCurrentPage(1); } }, [wishlistItems, currentPage, totalPages]);

  // --- SEO Data ---
  const pageTitle = "My Wishlist | GNT Store";
  const pageDescription = "View and manage your saved items on GNT Store. Keep track of products you love.";
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0f1115] text-white">
         <SEO
            title="Loading Wishlist... | GNT Store"
            description="Loading your saved wishlist items."
            noIndex={true} // Prevent indexing while loading
         />
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[#0f1115] text-white">
         <SEO
            title="Log In to View Wishlist | GNT Store"
            description="Please log in to view your saved wishlist items."
            noIndex={true} // Prevent indexing login prompt
         />
        <main className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-8">
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white">
       <SEO
            ...
            noIndex={true} // Prevent indexing of personal wishlist pages
            ogData={{ // OG data specific to wishlist
                title: pageTitle,
                description: pageDescription,
                url: canonicalUrl,
                type: 'website', // or profile? website is safer
      ...
      <main className="container mx-auto px-4 py-8">
        <motion.div className="max-w-4xl mx-auto" variants={staggerContainer} initial="hidden" animate="visible">
          <div className="flex items-center justify-between mb-8"> <h1 className="text-2xl md:text-3xl font-bold">My Wishlist</h1> <div className="flex flex-col sm:flex-row items-end sm:items-center gap-4"> {wishlistItems.length > 0 && ( <Button variant="ghost" className="text-gray-400 hover:bg-[#2e3044] hover:text-red-500" onClick={clearWishlist}> Clear All </Button> )} <Button variant="link" className="text-gray-400 hover:text-white px-0" onClick={continueShopping}> <ArrowLeft className="mr-2 h-4 w-4" /> Continue Shopping </Button> </div> </div>
          <div className="mb-4 text-sm text-gray-400"> {`Wishlist Items: ${wishlistItems.length}`} </div>
          <Separator className="my-4 bg-[#2a2d36]" />

          {wishlistItems.length === 0 ? (
            <motion.div variants={fadeIn} className="bg-[#1a1c23] border border-[#2a2d36] rounded-lg p-8 text-center"> <Heart className="w-16 h-16 mx-auto text-gray-500 mb-4" /> <h2 className="text-xl mb-4">Your wishlist is empty</h2> <p className="text-gray-400 mb-6">Add items to your wishlist while browsing our products.</p> <Button className="bg-[#5865f2] hover:bg-[#4752c4] text-white" onClick={continueShopping}> Browse Products </Button> </motion.div>
          ) : (
            <>
                ...
                {currentWishlistItems.map((item, index) => (
                  <motion.div key={item.id} variants={fadeIn} custom={index} transition={{ delay: index * 0.05 }} className="bg-[#1a1c23] border border-[#2a2d36] hover:border-[#5865f2] rounded-lg p-4 flex flex-col sm:flex-row items-center sm:items-start gap-4 cursor-pointer transition-colors duration-300" onClick={() => navigateToProduct(item.slug)}>
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row justify-between items-center sm:items-start w-full">
                      <div className="text-center sm:text-left mb-4 sm:mb-0"> <h3 className="font-medium text-lg line-clamp-2">{item.title}</h3> <div className="flex flex-col sm:flex-row gap-2 sm:items-center mt-1"> <p className="text-[#5865f2] font-bold"> {formatCurrencyWithSeparator(item.discount_price)} </p> {item.price > item.discount_price && ( <div className="flex items-center gap-2"> <p className="text-gray-400 line-through"> {formatCurrencyWithSeparator(item.price)} </p> <span className="bg-[#ff4d4d] text-white text-xs px-2 py-1 rounded"> {calculateDiscountPercentage(item.price, item.discount_price)}% OFF </span> </div> )} </div> </div>
              ...
              {totalPages > 1 && ( <div className="mt-6"> <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={handlePageChange} /> </div> )}
            </>
          )}
